...
class AppSettings {
  ...
  static Future<void> loadThemeMode() async {
    ...
    final value = prefs.getString(themeKey) ?? 'system';
...
class AppUi {
  ...
  static Color iconBg(BuildContext context) {
    return isDark(context) ? const Color(0xFF143A52) : AppColors.green.withOpacity(0.12);
  }
}
...
class _HomeScreenState extends State<HomeScreen> {
  static const String storageKey = 'packing_items_all_v2';
  static const String oldCustomStorageKey = 'custom_packing_items_v1';
  ...
  List<PackingItem> get filteredItems {
    return _items.where((item) => item.matches(_query)).toList();
  }
  ...
  List<PackingItem> _parseItems(String raw) {
  ...
  Future<void> _loadItems() async {
    ...
    final savedAll = prefs.getString(storageKey);

    if (savedAll != null && savedAll.trim().isNotEmpty) {
      try {
        final savedItems = _parseItems(savedAll);
        setState(() {
          _items = savedItems;
          _loading = false;
        });
    ...
    if (oldCustom != null && oldCustom.trim().isNotEmpty) {
      try {
        final oldItems = _parseItems(oldCustom);
            ...
            Padding(
              ...
              child: TextField(
                ...
                decoration: InputDecoration(
                  ...
                  prefixIcon: Icon(Icons.search, color: AppUi.muted(context)),
                  suffixIcon: _query.isNotEmpty
                      ? IconButton(
                          ...
                          icon: Icon(Icons.close, color: AppUi.muted(context)),
                        )
                      : null,
                  filled: true,
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        boxShadow: [
          BoxShadow(
            color: Colors.black.withOpacity(AppUi.isDark(context) ? 0.35 : 0.18),
            blurRadius: 18,
...
class _AddProductScreenState extends State<AddProductScreen> {
  ...
  void _save() {
    ...
    final category = widget.existingItem?.category ?? _guessCategory(title);
              ...
              Container(
                ...
                child: Row(
                  children: [
                    ...
                    const Expanded(
                      child: Text(
                        'Uygulama görünümünü ve kullanım ayarlarını buradan değiştirebilirsin.',
                        style: TextStyle(
                          color: Colors.white,
                          fontSize: 16,
                          fontWeight: FontWeight.w700,
              ...
              Container(
                ...
                child: Row(
                  children: [
                    ...
                    Expanded(
                      child: Text(
                        'Sonraki ayarlara yazı boyutu, sıkı/rahat liste görünümü ve yedekleme ekleyebiliriz.',
                        style: TextStyle(
                          color: AppUi.muted(context),
                          fontWeight: FontWeight.w600,
                        ),
