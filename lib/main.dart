...
class AppSettings {
  ...
  static Future<void> loadAll() async {
    ...
    final sortValue = prefs.getString(sortModeKey) ?? 'newestFirst';
    ...
    sortModeNotifier.value = _sortModeFromString(sortValue);
  }
...
class _HomeScreenState extends State<HomeScreen> {
  ...
  List<PackingItem> _sortedItems(List<PackingItem> source, SortMode mode) {
    ...
    switch (mode) {
      case SortMode.newestFirst:
        ...
        break;
      case SortMode.alphabetical:
        result.sort((a, b) => _normalize(a.title).compareTo(_normalize(b.title)));
        break;
      case SortMode.category:
        result.sort((a, b) {
          final category = _normalize(a.category).compareTo(_normalize(b.category));
  ...
  Future<void> _updateItem(PackingItem updatedItem) async {
    await BackupHelper.saveAutoBackup(_items);
...
class _BulkAddScreenState extends State<BulkAddScreen> {
  ...
  Future<void> _importProducts() async {
    ...
    final savedItems = await BackupHelper.readSavedItems();
    await BackupHelper.saveAutoBackup(savedItems);
    ...
    await BackupHelper.saveItems(merged);
...
class SettingsScreen extends StatelessWidget {
  ...
  Future<void> _restoreAutoBackup(BuildContext context) async {
    final raw = await BackupHelper.readAutoBackupCode();
    ...
    try {
      ...
      final current = await BackupHelper.readSavedItems();
      await BackupHelper.saveAutoBackup(current);
        ...
        children: [
          Container(
            ...
            child: Row(
              children: [
                ...
                const Expanded(
                  child: Text(
                    'Uygulama görünümünü, listeyi, toplu ürün eklemeyi ve yedekleri buradan yönetebilirsin.',
                    style: TextStyle(
                      color: Colors.white,
                      fontSize: 16,
                      fontWeight: FontWeight.w700,
          ...
          Container(
            ...
            child: ListTile(
              ...
              subtitle: Text(
                'Başlık = ... formatındaki metni otomatik ürüne çevirir.',
                style: TextStyle(
                  color: AppUi.muted(context),
                  fontWeight: FontWeight.w500,
                ),
          ...
          Container(
            ...
            child: ListTile(
              ...
              subtitle: Text(
                'Silme, düzenleme, çoğaltma veya geri yükleme öncesi alınan son yedeği geri getirir.',
                style: TextStyle(
                  color: AppUi.muted(context),
                  fontWeight: FontWeight.w500,
                ),
...
class _RestoreBackupScreenState extends State<RestoreBackupScreen> {
  ...
  Future<void> _restore() async {
    ...
    try {
      ...
      final currentItems = await BackupHelper.readSavedItems();
      await BackupHelper.saveAutoBackup(currentItems);
      await BackupHelper.saveItems(items);
